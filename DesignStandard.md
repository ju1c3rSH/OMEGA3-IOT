...
## 3. 每种类型的Properties
```text
1 :{Location,Temperature} //其中内容均使用Json表示（Altitude.....）
```
...
### device 对象字段
- `id` (int): 设备在数据库中的唯一数字 ID
- `uuid` (string): 设备全局唯一标识符（UUID v4 格式）
- `reg_code` (string): 8 位注册码，由大小写字母、数字、符号组成，用于用户绑定设备
- `verify_code` (string): 16 位校验码，由大小写字母、数字、符号组成，用于设备上传数据时的身份验证
- `type` (int): 设备类型 ID（1 = 基础测试用定位器）
- `expires_at` (int): 凭证过期时间（Unix 时间戳，秒级），示例值 1760858932 对应 UTC 时间 2025-10-18 15:28:52
